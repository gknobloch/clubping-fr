...
const ACTIVE_ONLY = 'Joueurs actifs uniquement'
...
test.describe('Club admin — Joueurs list', () => {
  ...
  test('shows a club header and Ajouter un joueur inside it, defaults to actifs only', async ({ page }) => {
    ...
    await expect(page.getByRole('checkbox', { name: ACTIVE_ONLY })).toBeChecked()
  ...
  test('a non-active player is hidden by default and shows a status badge once the toggle is off', async ({ page }) => {
    ...
    await page.getByRole('checkbox', { name: ACTIVE_ONLY }).check()
    await expect(page.getByText('Test Archivedplayer')).toHaveCount(0)
  })
...
// #438 — the directory is the active roster. A member gets no control to widen
// it, because for them there is nothing on the other side of it.
test.describe('Player — the archived members are not theirs to see', () => {
  test('has no toggle and no archived row', async ({ page }) => {
    await loginAs(page, 'szulc')
    await page.goto('/joueurs')

    await expect(page.getByRole('heading', { name: 'Joueurs' })).toBeVisible()
    await expect(page.getByRole('checkbox', { name: ACTIVE_ONLY })).toHaveCount(0)
    await expect(page.getByRole('combobox', { name: 'Statut' })).toHaveCount(0)
    await expect(page.getByText('Archivé')).toHaveCount(0)
  })
})
