...
//
// Unlike apiv2's divisionPoolsQuery, xml_result_equ.php only returns one pool
// at a time — `D1` alone (no `cx_poule`) always returns just the division's
// first pool, never the whole division (confirmed empirically; an earlier
// draft of this file assumed otherwise). Reaching a specific pool requires
// its dafunker-space id as `cx_poule`, which only ever aligns with local
...
//
// xml_equipe.php resolves that id-space gap authoritatively: given a club's
// FFTT affiliation number (which every local Club already carries), it lists
// every one of that club's teams together with the exact (D1, cx_poule) pair
// for their division/poule — no guessing needed. Fetching is therefore
// three-tier (see fetchGamesPreview in DataContext.tsx): look up cx_poule via
// xml_equipe.php using an affiliation number from one of the group's own
// teams (correct whenever available); else try `cx_poule=<Group.id>`
// directly (correct for #229-origin groups); else fall back to apiv2's
// whole-division fetch. dafunker validates that `cx_poule` actually belongs
// to the given `D1` (a mismatched pair returns an empty `<liste/>`, not
// another division's data), so a wrong guess only ever comes back empty —
// never a false positive.
//
// Sources:
//   GET https://fftt.dafunker.com/v1/proxy/xml_result_equ.php?force=1&D1=<FFTT division id>[&cx_poule=<dafunker pool id>]
...
//     </tour>...</liste>
//   GET https://fftt.dafunker.com/v1/proxy/xml_equipe.php?force=1&numclu=<affiliation number>
//     <liste><equipe><idepr/><libepr/><idequipe/><libequipe>RIXHEIM PPA 1 - Phase 1</libequipe>
//       <libdivision>GE Elite P1 Poule 3</libdivision>
//       <liendivision><![CDATA[cx_poule=1502291&D1=234142&organisme_pere=14]]></liendivision>
//     </equipe>...</liste>
//     (dafunker's own site calls a same-purpose JSON endpoint at
//     /v1/club/<affiliation>/equipes, but it sends no CORS header and can't
//     be fetched from the browser; this /v1/proxy/ XML twin does.)
...
const EQUIPE_URL = 'https://fftt.dafunker.com/v1/proxy/xml_equipe.php'
...
/** The dafunker URL listing a club's teams, each with their division/poule (D1, cx_poule). */
export function dafunkerClubTeamsUrl(affiliationNumber: string): string {
  const safe = affiliationNumber.replace(/[^0-9A-Za-z]/g, '')
  return `${EQUIPE_URL}?force=1&numclu=${safe}`
}

/** One of a club's teams' division/poule, resolved to dafunker's own ids. */
export interface DafunkerClubTeamPool {
  divisionId: string
  cxPoule: string
  poolNumber: number | null
}

const POULE_NUMBER_RE = /Poule\s+(\d+)/i

/** Parse an xml_equipe.php response into each team's (D1, cx_poule, poule number). */
export function parseDafunkerClubTeamsXml(xml: string): DafunkerClubTeamPool[] {
  const doc = new DOMParser().parseFromString(unwrapCdata(xml), 'application/xml')
  if (doc.querySelector('parsererror')) return []

  return Array.from(doc.querySelectorAll('equipe')).flatMap((eq) => {
    const params = new URLSearchParams(text(eq, 'liendivision'))
    const divisionId = params.get('D1')
    const cxPoule = params.get('cx_poule')
    if (!divisionId || !cxPoule) return []
    const poolNumber = text(eq, 'libdivision').match(POULE_NUMBER_RE)
    return [{ divisionId, cxPoule, poolNumber: poolNumber ? Number(poolNumber[1]) : null }]
  })
}
