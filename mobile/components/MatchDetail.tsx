...
export function MatchDetail({
  ...
  gameId: string
  teamId: string
  /** L'axe d'où l'on vient (#552) — sans objet dans un volet, qui a sa liste. */
  from?: string
  /**
   * Rendu dans le volet droit de « Tous les matchs » (#585) plutôt que poussé.
   * Un volet ne nomme pas l'écran (il n'y en a pas à nommer) et ne feuillette
   * pas : le rail à côté *est* la liste des matchs, donc un second geste pour
   * la même chose mettrait deux sens sur un même balayage.
   */
  embedded?: boolean
}) {
  ...
  useEffect(() => {
    if (matchDay && !embedded) navigation.setOptions({ title: `Journée ${matchDay.number}` })
  }, [matchDay, navigation, embedded])
  ...
  return (
    <Screen pane={embedded}>
      <View style={styles.pan} {...(embedded ? {} : swipe)}>
        ...
        <ScrollView key={game.id} contentContainerStyle={[styles.scroll, contentWidth()]}>
          ...
          {/* Under the card, where the accueil's carousel puts its own dots —
              ...
              below. */}
          {pager && <GamePager neighbours={pager} />}
          ...
          {/* Jump to the team's full phase view (roster + all matches).
              Absent dans un volet : c'est l'écran où l'on se trouve déjà, et
