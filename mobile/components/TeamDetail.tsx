...
// La fiche équipe (#447)
//
// This was the `(detail)/team/[id]` screen and nothing else. It is a component
// now because a tablet shows it *beside* the Équipes list rather than pushed
// over it, and the same fiche has to serve both — a second, pane-shaped copy
// would drift from this one within a release.
//
// Only two things depend on which of the two it is, both collected in
// `embedded`: the header title (there is no pushed screen to name), and the
// safe-area frame (the pane sits inside one already).
// ---------------------------------------------------------------------------
export function TeamDetail({
  ...
  useEffect(() => {
    if (team && !embedded) navigation.setOptions({ title: getTeamName(team, clubs) })
  }, [team, clubs, navigation, embedded])
